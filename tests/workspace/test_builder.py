...
def test_global_session_env_options(session: Session, monkeypatch: MonkeyPatch) -> None:
    visual_silence = "on"
...
def test_find_current_active_pane(server: Server, monkeypatch: MonkeyPatch) -> None:
    workspace = ConfigReader._from_file(
        path=test_utils.get_workspace_file("workspace/builder/three_windows.yaml")
    )
